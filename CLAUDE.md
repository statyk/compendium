...
- **Password-change UI** — `AuthService.set_password` + `compendium user set-password` CLI ship with the Docker slice, but the web UI has no self-service or admin password-change flow yet. After a Docker deployment, the default `admin:compendium` credentials can only be changed from inside the container (`docker compose exec compendium compendium user set-password ...`), which is friction. Needed: a patron self-service "change my password" on `/ui/me/*` and a librarian-only reset on `/ui/users/{username}`.
