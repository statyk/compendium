...
4. **Current status (last updated 2026-04-18):** slices 1–9b.1+9b.2 complete. Slice 9b.1 added patron↔user linking: `PatronService.link_user/unlink_user` enforce 1:1 at service level; web dropdown on patron create form; link/unlink actions on patron detail; CLI `patron --link-user`, `patron link-user`, `patron unlink-user`. Slice 9b.2 added user management web: `/ui/users` list, `/ui/users/new` create form, `/ui/users/{username}` detail with role change (form POST + redirect) and HTMX deactivate; `AuthService.update_role/list_users`; `SqlUserRepository.list()`; `SqlRoleRepository.list()`. "Users" nav link added for Librarians. New web routes: `GET /users`, `GET/POST /users/new`, `GET /users/{username}`, `POST /users/{username}/change-role`, `POST /users/{username}/deactivate`. `Patron.user` relationship added to domain model. CLI covers: `audit list`, `db init`, `item add --isbn`, `item show/list`, `item withdraw --barcode`, `patron add [--link-user]`, `patron link-user`, `patron unlink-user`, `patron list`, `patron deactivate --card`, `loan checkout/checkin/renew/active`, `hold place/cancel/list`, `policy list/set`, `maintenance expire-holds`, `user add`, `user deactivate --username`, `serve`. FastAPI routes: `POST /auth/login`, `GET /works/search`, `GET /items/{barcode}`, `POST /items/{barcode}/withdraw`, `POST /patrons`, `POST /patrons/{card_number}/deactivate`, `POST /loans/checkout`, `POST /loans/{id}/checkin`, `POST /loans/{id}/renew`, `POST /holds`, `GET /holds`, `DELETE /holds/{id}`, `GET /policies`, `POST /policies`, `POST /users/{username}/deactivate`. Self-service: `GET /me/loans`, `GET /me/holds`, `POST /me/holds`, `DELETE /me/holds/{id}`, `POST /me/loans/{id}/renew`. Web UI at `/ui/*`: login/logout, catalog search, work detail + place-hold + add-copy, item detail + withdraw, add item by ISBN + scanner, circ desk + scanner, patron list + create (with user link dropdown) + detail (with link/unlink) + deactivate, patron self-service loans/holds, user list + create + detail (role change + deactivate), audit log viewer. Key implementation notes: `Patron.user` is a lazy-loaded relationship (no back_populates). Patron/user detail pages accept `?message=` and `?error=` query params for post-redirect feedback. `_unlinked_users()` helper in patron routes queries patron.user_id to filter used accounts. SQLite backend only. 135 tests passing.
5. Logical next steps (discuss with user before starting): slice 9b.3+9b.4 (policy management web, role management web), additional media types / metadata sources, MARC/CSV bulk import, Postgres backend support.
